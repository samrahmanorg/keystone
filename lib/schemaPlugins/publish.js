		...
		if(publishTo && publishTo.trim().length > 0) {
			publishRequest(publishTo, doc, function(error, result){
				if(error){
					...
					if(publishTo == "sandbox"){
						doc.set(options.sandbox, now)

						if (options.sandboxBy && user) {
							console.log("inside sandboxBy check")
							doc.set(options.sandboxBy, user._id);
						}
						else {
							console.log("no sandboxBy")
						}
					}
					else if(publishTo == "dev"){
