...
function render(req, res, view, ext) {
	...
	// hide the "users" section if not super admin
	var hideUsersSection = function() {
		//console.log("nav====>", JSON.stringify(keystone.nav, null, 2));
		var navCloned = _.clone(keystone.nav);
		var user = req.user;
		if(user && !user.isSuperAdmin){
			var markedIndex = -1;

			// clone the nav.sections array
			var sectionsCloned = navCloned.sections.slice(0);

			for(var i = 0; i < sectionsCloned.length; i++){
				var label = sectionsCloned[i].label;
				if(label == 'Users') {
					markedIndex = i;
					break;
				}	
			...
			if(markedIndex != -1){
				sectionsCloned.splice( markedIndex, 1 );
				navCloned.sections = sectionsCloned
			}
		}
		return navCloned;
	}
	var navWithoutUsers = hideUsersSection();
	...
	var locals = {
		...
		nav: navWithoutUsers, //nav: keystone.nav,
