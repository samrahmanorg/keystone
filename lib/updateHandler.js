...
UpdateHandler.prototype.process = function(data, options, callback) {
	...
	options.fields.forEach(function(path) {
		...
		switch (field.type) {
			...
			case 'ddgoogleplace':
				actionQueue.push(field.getRequestHandler(this.item, this.req, options.paths, function(err, duplicateEntry) {
					if (err && options.flashErrors) {
						this.req.flash('error', field.label + ' improve failed - ' + (err.status_text || err.status));
					}
					if(duplicateEntry && !err) {
						err = new Error('duplicate entry in mongo for that place, place not added');
					}
					progress(err);
				}.bind(this)));
		...
		if(data["publishTo"] && data["publishTo"].trim().length > 0){
			this.item.publishTo = data["publishTo"].trim();
		}

		// publishing indicator
		if(data["publishToConnectEnv"] && data["publishToConnectEnv"].trim().length > 0){
			this.item.publishToConnectEnv = data["publishToConnectEnv"].trim();
		}
